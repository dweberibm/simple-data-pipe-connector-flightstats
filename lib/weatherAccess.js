...
//			"&language=en-US";
		var url = weatherService.credentials.url + "/api/weather/v1/geocode/" + airportInfo.latitude.toFixed(2) + "/" + airportInfo.longitude.toFixed(2) + "/observations/timeseries.json?hours=23";
		request.get(url, {"json":true},
			function(err, response, body){
				if ( err || body.error || response.statusCode != 200 ){
					console.log("Unable to get weather timeSeries info for url %s ",url, err || body.error || ( "Code: " + response.statusCode + " : " + response.statusMessage ) );
					return callback(err);
				}
				...
				if ( !_.isArray(observations) ){
					return callback("Invalid results from weather api: ", body);
				}
